...
// Liquify/Warp handle-drag capture (D3.5). Enabled from DevelopPanel: drag creates warp
// (center = click point, displacement vector = drag direction). Each warp renders an arrow + radius circle.
public partial class CenterPreview
{
    ...
    private Point _liquifyStart;   // click point (overlay coordinates)

    /// <summary>Bind DevelopPanel to toggle liquify and redraw handles on image/resize change.</summary>
    ...
    private void LiquifyOverlay_MouseMove(object sender, MouseEventArgs e)
    {
        ...
        var p = e.GetPosition(liquifyOverlay);
        DrawLiquifyOverlay();                  // committed handles
        DrawArrow(_liquifyStart, p, true);     // active drag arrow (preview)
    }
    ...
    private void LiquifyOverlay_MouseUp(object sender, MouseButtonEventArgs e)
    {
        ...
        // Center = click point (normalized per axis); delta = drag vector scaled by LONG EDGE.
        ...
        // Ignore tiny drag gestures (prevent accidental clicks).
    ...
    /// <summary>Redraw all committed warp handles (center + displacement arrow + radius circle).</summary>
    ...
    // Draw arrow from a -> b (preview=true: yellow during drag; false: green committed).
