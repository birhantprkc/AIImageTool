...
public static class ContactSheet
{
    ...
    public static int RenderPdf(IReadOnlyList<string> imagePaths, string outPath, Options opt,
        string documentTitle = "ZeroVision Photo Catalog")
    {
        ...
        for (int p = 0; p < totalPages; p++)
        {
            ...
            string footerLeft = "ZeroVision Studio - Powered by ZeroReports";
    ...
    private static Image<Rgba32>? LoadThumb(string path, int w, int h)
    {
        ...
        if (ZeroVision.Imaging.RawPreviewExtractor.IsRawExtension(path))
        {
            var jpeg = ZeroVision.Imaging.RawPreviewExtractor.ExtractLargestJpeg(path);
