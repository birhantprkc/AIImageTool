...
public partial class MainWindow : Window
{
    ...
    private ZVision.Shared.FolderWatcher? _folderWatcher;
        ...
        ImageToolHostProvider hostProvider)
    {
        ...
        developPanel.SubjectMaskRequested += async (s, path) =>
        {
            ...
            {
                ZVision.Shared.AppLog.Error("MainWindow.SubjectMask", path, ex);
    ...
    private static string OpLabel(EditOperation op)
        => ZVision.Shared.OpDisplayNames.Get(op.OpType, op.Title);
    ...
    public void ShowToast(string message)
    {
        Dispatcher.BeginInvoke(() =>
        {
            try
            {
                ZeroUI.Wpf.Overlays.ToastNotification.Show(this, message, "ZVision",
    ...
    private void BtnMore_Click(object sender, RoutedEventArgs e)
    {
        ...
        miHdr.Click += (_, _) => ImageContextMenu.RunMerge(targets, ZVision.Shared.MergeService.Mode.Hdr);
        ...
        miFocus.Click += (_, _) => ImageContextMenu.RunMerge(targets, ZVision.Shared.MergeService.Mode.FocusStack);
        menu.Items.Add(miFocus);

        var miPano = new System.Windows.Controls.MenuItem { Header = "Panorama Stitcher", IsEnabled = targets.Count >= 2 };
        miPano.Click += (_, _) => ImageContextMenu.RunMerge(targets, ZVision.Shared.MergeService.Mode.Panorama);
    ...
    private async void FindDuplicates()
    {
        ...
        try
        {
            var decoders = ZVision.Imaging.ImageDecoderRegistry.CreateDefault();
            var groups = await System.Threading.Tasks.Task.Run(
                () => ZVision.Shared.DuplicateFinder.FindGroups(paths, decoders, 10));
    ...
    private async void ImportFromUrl()
    {
        ...
        if (!ZVision.Shared.UrlImageImporter.IsValidImageUrl(url, out _))
        {
        ...
        try
        {
            ...
            string saved = await ZVision.Shared.UrlImageImporter.DownloadAsync(url, destFolder, _urlHttp);
    ...
    private void ToggleWatchFolder()
    {
        ...
        _folderWatcher ??= new ZVision.Shared.FolderWatcher();
