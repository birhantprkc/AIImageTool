...
public partial class CenterPreview
{
    ...
    public void SwapCropOrientation()
    {
        ...
        var r = ZVision.Imaging.CropAspect.SwapOrientation(imgW, imgH, _cropX, _cropY, _cropW, _cropH);
        _cropX = r.X;
        _cropY = r.Y;
        _cropW = r.W;
        _cropH = r.H;
    ...
    public void BindCropPanel(DevelopPanel panel)
    {
        ...
        {
            foreach (var p in ZVision.Imaging.CropAspect.Presets)
    ...
    private void CmbCropRatio_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
    {
        ...
        else
        {
            ...
            var r = ZVision.Imaging.CropAspect.Centered(bs.PixelWidth, bs.PixelHeight, preset.Item2, preset.Item3);
    ...
    private void BtnSmartCrop_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            ZVision.Shared.AppLog.Warn("CenterPreview.SmartCrop", $"{path}: {ex.Message}");
        }
    }
    ...
    /// </summary>
    private static System.Collections.Generic.IReadOnlyList<ZVision.Core.EditOperation> StripCropRect(
        System.Collections.Generic.IReadOnlyList<ZVision.Core.EditOperation> ops, int pointer)
    {
        var result = new System.Collections.Generic.List<ZVision.Core.EditOperation>(ops.Count);
        ...
        for (int i = 0; i < ops.Count; i++)
        {
            ...
            if (i < n && string.Equals(op.OpType, "Crop", StringComparison.OrdinalIgnoreCase))
            {
                ...
                result.Add(new ZVision.Core.EditOperation { PluginId = op.PluginId, OpType = op.OpType, Title = op.Title, Params = p });
            }
            else result.Add(op);
        }
        return result;
